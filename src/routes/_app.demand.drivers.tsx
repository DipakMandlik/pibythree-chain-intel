import { createFileRoute, Link } from "@tanstack/react-router";
import { Button } from "@/components/ui/button";
import { PageHeader, SectionCard } from "@/components/app/page-header";
import { AiInsightCard } from "@/components/app/ai-insight-card";
import { SeverityBadge } from "@/components/app/badges";
import {
  demandDriverContribution,
  demandSignals,
  forecastSignals,
  skuById,
  storeById,
} from "@/data/demo";
import { pageMeta } from "@/lib/seo";

export const Route = createFileRoute("/_app/demand/drivers")({
  head: () =>
    pageMeta(
      "Demand Drivers",
      "Historical trend, promotion, seasonality and local pattern contributions behind the forecast.",
    ),
  component: Drivers,
});

function Drivers() {
  return (
    <>
      <PageHeader
        title="Demand Drivers"
        subtitle="What is influencing demand — decomposed into historical trend, promotion, seasonal and local signals."
      />
      <div className="grid gap-5 xl:grid-cols-[1.3fr_1fr]">
        <SectionCard
          title="Portfolio driver contribution"
          description="Share of forecasted demand explained by each driver, across the demand portfolio."
        >
          <ul className="space-y-4">
            {demandDriverContribution.map((d) => (
              <li key={d.driver}>
                <div className="flex items-center justify-between text-sm">
                  <span className="font-medium text-navy">{d.driver}</span>
                  <span className="num font-semibold text-primary">{d.share}%</span>
                </div>
                <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-surface-strong">
                  <div
                    className="h-full rounded-full bg-primary"
                    style={{ width: `${d.share}%` }}
                  />
                </div>
                <p className="mt-1 text-xs text-muted-foreground">{d.note}</p>
              </li>
            ))}
          </ul>
        </SectionCard>
        <SectionCard
          title="Active signals — Beverages, Mumbai"
          description="Demonstration-generated uplift signals for the current forecast."
        >
          <ul className="space-y-4">
            {forecastSignals.map((d) => (
              <li key={d.driver}>
                <div className="flex items-center justify-between text-sm">
                  <span className="font-medium text-navy">{d.driver}</span>
                  <span className="num font-semibold text-primary">+{d.impact}%</span>
                </div>
                <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-surface-strong">
                  <div
                    className="h-full rounded-full bg-primary"
                    style={{ width: `${d.impact * 7}%` }}
                  />
                </div>
                <p className="mt-1 text-xs text-muted-foreground">{d.note}</p>
              </li>
            ))}
          </ul>
        </SectionCard>
      </div>

      <SectionCard
        title="Recent demand signals"
        description="Deviations detected by the AI engine in the last 7 days."
        padded={false}
      >
        <ul className="divide-y divide-border">
          {demandSignals.map((s) => (
            <li key={s.id} className="px-4 py-3">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-medium text-navy">{skuById(s.skuId).name}</p>
                <SeverityBadge severity={s.severity} />
              </div>
              <p className="mt-0.5 text-xs text-muted-foreground">
                {storeById(s.storeId).name} {storeById(s.storeId).code} · {s.interpretation}
              </p>
            </li>
          ))}
        </ul>
      </SectionCard>

      <AiInsightCard
        title="Raise Mumbai beverage forecast by 14% for the next 14 days"
        severity="medium"
        reason="Promotion uplift and warm-weather seasonality are compounding above baseline for a sustained period."
        evidence={[
          "Promotion impact +11%",
          "Recent trend +7%",
          "Seasonality +5%",
          "Local demand pattern +3%",
        ]}
        impact="Improves forecast accuracy by an estimated 2.1 points"
        confidence={86}
        actions={
          <>
            <Button asChild size="sm">
              <Link to="/demand/forecast-explorer">Open forecast explorer</Link>
            </Button>
            <Button asChild size="sm" variant="outline">
              <Link to="/ai/assistant">Ask Demand AI</Link>
            </Button>
          </>
        }
      />
    </>
  );
}
